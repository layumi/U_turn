...
data_dir = test_dir
image_datasets = {x: datasets.ImageFolder( os.path.join(data_dir,'query') ,data_transforms) for x in ['test']}
...
def generate_attack(model,dataloaders, method_id):
    ...
    for data in dataloaders:
        ...
        #4. Label-smooth method
            ...
            smooth_label = torch.ones(batch_size, 100) /100.0
        ...
        #6. PIRE
            ...
            if opt.iter ==20:
                opt.iter = round(min(1.25 * opt.rate, opt.rate+4))
            for iter in range(opt.iter): #round(min(1.25 * opt.rate, opt.rate+4))):
                optimizer.zero_grad()
        ...
        for j in range(inputs.shape[0]):
            ...
            dir_path = os.path.join('./attack_query', opt.name + '-' + str(opt.method_id), str(opt.rate), query_path[count+j][0].split('/')[-2])
            im = Image.fromarray(im.astype('uint8'))
            if not os.path.isdir(dir_path):
                os.mkdir(dir_path)
            im.save( os.path.join( dir_path, im_path) )
        count += n
        #print(count)
...
#print('-------generate-----------')
if opt.use_dense:
    model_structure = ft_net_dense(224)
else:
    model_structure = ft_net(224)
...
# Clip 
zeros = np.ones((256,256,3),dtype=np.uint8)
zeros = Image.fromarray(zeros) 
zeros = data_transforms(zeros)

ones = 255*np.ones((256,256,3), dtype=np.uint8)
...
generate_attack(model,dataloaders['test'],opt.method_id)

...
parser.add_argument('--method', default=0, type=int, help='test_image_index')
parser.add_argument('--test_dir',default='../Market/pytorch',type=str, help='./test_data')
parser.add_argument('--adv',action='store_true', help='./test_data')
opts = parser.parse_args()

data_dir = opts.test_dir
image_datasets = {x: datasets.ImageFolder( os.path.join(data_dir,x) ) for x in ['query','gallery']}
...
result = scipy.io.loadmat('query_result.mat')

if opts.adv:
    result = scipy.io.loadmat('attack_query/baseline-%d/16/query_result.mat'%opts.method)
...
result = scipy.io.loadmat('gallery_result.mat')
...
if opts.adv:
    query_path = query_path.replace('../Market/pytorch/query/%04d'%query_label,'attack_query/baseline-%d/16/'%opts.method)
print(query_path)
...
try: # Visualize Ranking Result 
    ...
    #query256 = query256.resize((256,128))
